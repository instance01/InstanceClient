...
public class RenderManager {
		...
		try {
			...
			if (var11 != null && this.renderEngine != null) {
				try {
					...
					// TODO t
					if (InstanceMain.getInstance().coloredmobs) {
	 ...
	 */
	private void renderDebugBoundingBox(Entity par1Entity, double par2, double par4, double par6, float p_85094_8_, float par9) {
		// TODO t
		float var10_ = par1Entity.width / 2.0F;
		AxisAlignedBB var11_ = AxisAlignedBB.fromBounds(par2 - (double) var10_, par4, par6 - (double) var10_, par2 + (double) var10_, par4 + (double) par1Entity.height, par6 + (double) var10_);
		...
		if (par1Entity instanceof EntityMob) {
			RenderGlobal.drawOutlinedBoundingBox(var11_, 16711680); // red
		} else if (par1Entity instanceof EntityPlayer) {
			if (!(par1Entity instanceof EntityPlayerSP)) {
				RenderGlobal.drawOutlinedBoundingBox(var11_, 6881434); // yellow
			}
		} else {
			RenderGlobal.drawOutlinedBoundingBox(var11_, 65280); // green
		}
